...
namespace Cowboy.WebSockets
{
    internal sealed class WebSocketClientHandshaker
    {
        ...
        internal static byte[] CreateOpenningHandshakeRequest(WebSocketClient client, out string secWebSocketKey)
        {
            ...
            StringBuilderExtensions.AppendFormatWithCrCf(sb,
                "GET {0} HTTP/{1}",
            ...
            StringBuilderExtensions.AppendFormatWithCrCf(sb, Consts.HttpHeaderLineFormat, HttpKnownHeaderNames.Host, client.Uri.Host);

            // The request MUST contain an |Upgrade| header field whose value
            // MUST include the "websocket" keyword.
            StringBuilderExtensions.AppendFormatWithCrCf(sb, Consts.HttpHeaderLineFormat, HttpKnownHeaderNames.Upgrade, Consts.WebSocketUpgradeToken);

            // The request MUST contain a |Connection| header field whose value
            // MUST include the "Upgrade" token.
            StringBuilderExtensions.AppendFormatWithCrCf(sb, Consts.HttpHeaderLineFormat, HttpKnownHeaderNames.Connection, Consts.WebSocketConnectionToken);
            ...
            StringBuilderExtensions.AppendFormatWithCrCf(sb, Consts.HttpHeaderLineFormat, HttpKnownHeaderNames.SecWebSocketKey, secWebSocketKey);

            // The request MUST include a header field with the name
            // |Sec-WebSocket-Version|.  The value of this header field MUST be 13.
            StringBuilderExtensions.AppendFormatWithCrCf(sb, Consts.HttpHeaderLineFormat, HttpKnownHeaderNames.SecWebSocketVersion, Consts.WebSocketVersion);
            ...
            StringBuilderExtensions.AppendWithCrCf(sb);
            ...
            out List<string> protocols)
        {
            ...
            var lines = response.Split(_headerLineSplitter);
                        ...
                        if (line.StartsWith(key + ":"))
                        {
                            ...
                            if (index != -1)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        headers[key] = headers[key] + "," + value.Trim();
                                    }
                                    else
                                    {
